...
class WHUPipeline(EvalPipeline):
    default_conf = {
        "data": {
            "batch_size": 1,
            "name": "homography_dataset",
            "data_dir": "InvModal/WHU-OPT-SAR/WHU_homography/rgb_nirs",
            "num_workers": 16,
            "preprocessing": {
                "resize": 540,  # we also resize during eval to have comparable metrics
                "side": "short",
            },
        },
        "model": {
    ...
    def get_dataloader(self, data_conf=None):
        ...
        dataset = get_dataset(data_conf.get("name", "homography_dataset"))(data_conf)
    ...
    def run_eval(self, loader, pred_file):
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            if "est_H" in list(pose_results.values())[0]:
                results_i["est_H"]=list(pose_results.values())[0]["est_H"][0]
            # results_i["q_idx"]= data["q_idx"]
        ...
        auc_ths = [1, 3, 5,10,15,20]
        # pck_th=[0.01,0.03,0.05]
        # best_pose_results, best_th = eval_poses(
        #     pose_results, auc_ths=auc_ths, key="H_error_ransac", unit="px",pck_th=pck_th
        # )
        best_pose_results, best_th = eval_poses(
...
if __name__ == "__main__":
    ...
    default_conf = OmegaConf.create(WHUPipeline.default_conf)
